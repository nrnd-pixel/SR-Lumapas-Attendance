...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, Phase 3B5, Phase 3B6, and Phase 3B7A are complete and merged. Phase 3B7B has not started.
...
- **3B7B+ — Remaining ownership split:** after 3B7A is separately verified and merged, impact-map teacher signup/gate/pending-signup/`enterApp()` ownership as its own startup-sensitive checkpoint, then statistics/dashboard/reporting ownership, and finally orchestration/event wiring in small reversible checkpoints. Do not combine dependency migration, CSP hardening, backend security changes, UI redesign, stale-roster/teacher-list correctness work, or reporting consolidation with these structural moves.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B7A baseline has 44 Chromium tests, including the teacher-admin hostile-text/action regression.
...
**Current checkpoint:** Phase 3B7A — COMPLETE AND MERGED. Phase 3B7B has not started.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `f9fb4f88a98f5e9f03a1d33b92ebe69ea19691ca`, the Phase 3B7A runtime merge from PR #33. Its parents are previous signed `main` `fbe65f87b5a6a106ce1953a9d22a26a6793c7635` and approved normalized PR head `30228093fb1e013929bad99ccf76a12505fae0dc`.
- PR #33 exact approved head `30228093fb1e013929bad99ccf76a12505fae0dc` was exactly one commit ahead / zero behind with five changed files and passed Phase 0A Integrity `34862335422`, Phase 0B Backend Contract `34862335187`, and Phase 1 Playwright `34862335217`; Playwright ran 44 Chromium tests with all 44 passing in 17.4s on Node `v22.23.2`, npm `10.9.8`, with 0 vulnerabilities.
- The merged Phase 3B7A tree is identical to the final normalized PR tree `63c09c25d0ee6f72b376c46aa6784b49b6ba0c70`. The new teacher-admin hostile-text/action regression, all existing teacher-admin/signup/recovery tests, all ten Attendance tests, student-management tests, and February/missing-register/dashboard/Term 1 + CSV safeguards passed.
- Phase 3B7A merged only `assets/js/teacher-admin.js`, `assets/js/main.js`, `tests/e2e/admin-management.spec.mjs`, `.github/workflows/phase0a-integrity.yml`, and this roadmap. `index.html`, signup/gate/recovery ownership, SQL/RPC/grant/RLS, hosted Auth, Science, Netlify, production data, and deployment remained unchanged.
- Fresh Phase 3B7A live read-only verification confirmed the four teacher-admin RPCs remain authenticated-only SECURITY DEFINER functions with internal admin checks; latest Attendance migration remains `20260906125521 attendance_v12_term_ytd_reporting`. No Supabase mutation was made.
- Newly confirmed direct-DML/audit-bypass risk on teacher membership/assignment tables remains deferred to Phase 5. The pre-existing teacher-list stale-response race remains a separate correctness follow-up. Active assignment-role collapse to generic `teacher` remains a Phase 5 design/security item. None was mixed into 3B7A.
...
**Next action:** after this documentation-only Phase 3B7A closure is merged, perform a fresh Phase 3B7B impact map from the then-current signed `main`. Do not implement Phase 3B7B until that map is reviewed and explicitly approved.
