...
**Status:** In progress. Phase 3A and Phase 3B1 are complete and merged. Phase 3B2 is implemented on its cleanup branch and merge pending. Phase 3B3 has not started.
...
- Phase 0A now preserves the original frozen v1.0 checksum, separately proves the historical Phase 3A extraction and historical Phase 3B1 client boundary, and verifies Phase 3B2 as an exact transformation of the merged Phase 3B1 runtime: only shared `state`, `requestSerial`, `beginRequest()`, and `isLatestRequest()` move to `assets/js/app-state.js`; `index.html` and `supabase-client.js` remain unchanged; all three current JavaScript modules are syntax-checked.
...
**Next action:** complete exact-current-head Phase 3B2 CI, diff/review verification, and explicit merge approval. Do not begin Phase 3B3 automatically.
