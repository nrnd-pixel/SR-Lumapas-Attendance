...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, and Phase 3B5 are complete and merged. Phase 3B6 has not started.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B5 baseline now has 41 Chromium tests, including the three attendance/correction regressions for untouched `PP`/`TM` preservation, required `OTHER` notes, and non-school-day save blocking.
...
**Current checkpoint:** Phase 3B5 — COMPLETE AND MERGED VIA PR #28. Phase 3B6 has not started.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `8a3b8173a1e576a8eee39df721bcfcc7a98ce971`, the Phase 3B5 runtime merge.
- Approved PR #28 head: `6cf10229ca8da5d8fb012dc483f7d36250a1010e`; merge parents are `dc274130989e32d354ff4f28d8cc3c302010b7e5` and `6cf10229ca8da5d8fb012dc483f7d36250a1010e`.
- Phase 3B5 changed exactly five files: added `assets/js/attendance-register.js`; modified only `assets/js/main.js`, `tests/e2e/attendance.spec.mjs`, `.github/workflows/phase0a-integrity.yml`, and this roadmap. `index.html`, `assets/js/supabase-client.js`, `assets/js/app-state.js`, `assets/js/date-helpers.js`, `assets/js/ui-helpers.js`, `assets/js/auth-session.js`, `netlify.toml`, repository SQL, and deployment configuration remained unchanged.
- Exact approved-head CI for PR #28: Phase 0A Integrity `34808420449` — success; Phase 0B Backend Contract `34808420379` — success; Phase 1 Playwright `34808420455` — success with 41/41 Chromium tests in 17.1s, Node `v22.23.2`, npm `10.9.8`, and 0 vulnerabilities.
- All ten Attendance browser tests passed by name, including the three Phase 3B5 regressions for untouched `PP`/`TM` preservation, required `OTHER` note handling, and non-school-day save blocking.
- Reporting safeguards remained green: 3A February official 17-day invariant, monthly missing-register denominator semantics, admin-dashboard missing-register-not-zero semantics, and the 3A Term 1 45/45 invariant plus CSV values.
- Fresh read-only live Supabase verification during the Phase 3B5 impact map confirmed `attendance_load_register` and `attendance_save_register` remain authenticated-only SECURITY INVOKER RPCs using the existing `attendance.can_access_class` SECURITY DEFINER authorization helper. Correction/audit safeguards remain intact; known authenticated direct-DML grants remain deferred to Phase 5.
- No Supabase SQL/RPC/grant/RLS, hosted Auth, Science, Netlify, production-data, or deployment mutation was made for Phase 3B5.
