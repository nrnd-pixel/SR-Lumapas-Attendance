...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, Phase 3B5, Phase 3B6, Phase 3B7A, and Phase 3B7B are complete and merged. Phase 3B7C+ has not started.
...
- **3B7C+ — Remaining ownership split:** after 3B7B is separately verified and merged, impact-map statistics/dashboard/reporting ownership, and finally orchestration/event wiring, in small reversible checkpoints. Do not combine dependency migration, CSP hardening, backend security changes, UI redesign, stale-roster/teacher-list/access-check correctness work, or reporting consolidation with these structural moves.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B7B baseline has 45 Chromium tests including the rejected-request correction/resubmission regression. Exact PR #35 normalized head verification passed before merge.
...
**Current checkpoint:** Phase 3B7B — COMPLETE AND MERGED. Phase 3B7C+ has not started.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `326e54ea0c529f538e68433f2f30b4588ec2fe71`, the Phase 3B7B runtime merge from PR #35. Its parents are prior signed cleanup `main` `3259ad542b9b1816ab98aafa2a502b833ee69a65` and approved normalized PR head `69af4e3d8975f5a728223abd78bc3d5baca897fb`; GitHub signature verification is valid.
- PR #35 approved normalized head `69af4e3d8975f5a728223abd78bc3d5baca897fb` was exactly one commit ahead / zero behind its signed base with exactly five changed files. Exact-head CI passed: Phase 0A Integrity `34906180110`, Phase 0B Backend Contract `34906180159`, and Phase 1 Playwright `34906180122`; Playwright ran 45 Chromium tests with all 45 passing in 15.5s on Node `v22.23.2`, npm `10.9.8`, with 0 vulnerabilities.
- The rejected-request browser regression proves hostile-looking admin-note markup remains literal text, the rejected request pre-fills name/class/role correctly, the corrected class/role is sent in the exact `attendance_submit_teacher_request` payload, and the UI returns to Pending Approval. Existing signup isolation, recovery, all ten Attendance regressions, student/teacher admin, missing-register, 3A February, dashboard, Term 1/YTD, stale-report-response, CSV, and UI-helper coverage all remained green.
- Phase 3B7B extracts only teacher signup options/request submission, account-bound pending-signup storage, signup/gate rendering, and the teacher-authorization check into `assets/js/teacher-access.js`. `auth-session.js` remains unchanged; the authorized `attendance_bootstrap` application startup and event-listener registration remain in `main.js`.
- Fresh Phase 3B7B live read-only verification confirmed latest Attendance migration `20260906125521 attendance_v12_term_ytd_reporting`; `attendance_signup_options()` remains anonymous/authenticated SECURITY DEFINER by design, `attendance_submit_teacher_request(...)` and `attendance_teacher_status()` remain authenticated-only SECURITY DEFINER functions with caller-bound semantics, and `attendance_bootstrap()` remains authenticated SECURITY INVOKER. `teacher_signup_requests` and `attendance_private.teacher_access_audit` have no direct anon/authenticated table privileges. No Supabase mutation was made.
- The mapped Check Approval stale-response/UI-routing race remains a separate correctness follow-up. Existing student-roster and teacher-list stale-response gaps, teacher direct-DML/audit-bypass risk, and generic active-role semantics remain deferred to their recorded later checkpoints. None was changed by Phase 3B7B.
...
**Next action:** perform a fresh Phase 3B7C+ impact map from exact signed `main` `326e54ea0c529f538e68433f2f30b4588ec2fe71`, starting with statistics/dashboard/reporting ownership and choosing the smallest safe boundary from repository evidence. Do not implement until that map is reviewed and explicitly approved. Do not fix the Check Approval, student-roster, or teacher-list races; do not begin Phase 4 reporting consolidation or Phase 5 security changes inside the structural impact map.
