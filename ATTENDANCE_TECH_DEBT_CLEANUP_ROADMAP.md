...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, Phase 3B5, Phase 3B6, Phase 3B7A, and Phase 3B7B are complete and merged. Phase 3B7C is implemented on draft PR #37 with structural verification green; final five-file normalization, exact-head verification, final review, and explicit merge approval remain pending. Phase 3B7D+ has not started.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B7B baseline has 45 Chromium tests, and the verified Phase 3B7C structural head has 46/46 Chromium tests including the gender-incomplete monthly-statistics regression. Final normalized-head verification remains mandatory before merge.
...
**Current checkpoint:** Phase 3B7C — IMPLEMENTED ON DRAFT PR #37; structural verification is green. Final roadmap/five-file normalization, exact-head CI, final diff/review inspection, and explicit merge approval are pending. Phase 3B7D+ has not started.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `1912512eb344b17fa0f219268becfdac31bba00d`, the Phase 3B7B documentation-closure merge from PR #36. Its parents are signed Phase 3B7B runtime merge `326e54ea0c529f538e68433f2f30b4588ec2fe71` and approved documentation head `5360b2f20893f8274bb1b7c84ab7b2826e67ea58`; GitHub signature verification is valid.
- Draft PR #37 structural head `33db9b0429f0412731f79717473d61013660e508` is exactly one commit ahead / zero behind signed `main` and contains exactly four structural files: `assets/js/statistics.js`, `assets/js/main.js`, `tests/e2e/reporting.spec.mjs`, and `.github/workflows/phase0a-integrity.yml`. Structural CI passed: Phase 0A Integrity `34912869909`, Phase 0B Backend Contract `34912869882`, and Phase 1 Playwright `34912869880`; Playwright ran 46 Chromium tests with all 46 passing in 18.2s on Node `v22.23.2`, npm `10.9.8`, with 0 vulnerabilities.
- The new gender-incomplete browser regression proves the warning renders when gender is missing, male/female cumulative figures remain explicitly partial, total cumulative attendance remains complete at 398, possible attendance remains 425, the roster text remains correct, and the exact `attendance_monthly_class_stats` payload is unchanged. Existing 3A February, missing-register, dashboard, Term 1/YTD/CSV, stale-report-response, signup/recovery, all ten Attendance, student/teacher administration, and UI-helper coverage all remained green.
- Phase 3B7C moves only monthly statistics frontend ownership into `assets/js/statistics.js`. `state.monthlyStats` and `requestSerial.monthlyStats` remain in `app-state.js`; statistics panel orchestration and event listeners remain in `main.js`; dashboard and Term/YTD/CSV ownership remain untouched.
...
- Admin Dashboard is deliberately deferred to Phase 3B7D because it depends on `schoolId()`, admin authorization, and Dashboard → Statistics navigation. Term/YTD rendering and CSV/browser-download ownership are deliberately deferred to Phase 3B7E. The Check Approval, student-roster, and teacher-list stale-response gaps plus Phase 5 teacher direct-DML/role debt remain unchanged and out of scope.
...
**Next action:** add this roadmap as the fifth approved file, normalize the final five blobs directly onto signed base `1912512eb344b17fa0f219268becfdac31bba00d`, rerun Phase 0A/0B and the full 46-test Playwright suite on the exact normalized head, review the five-file diff/review surfaces, and stop for explicit merge approval. Do not start Phase 3B7D or 3B7E, fix the recorded stale-response races, begin Phase 4 reporting consolidation, or begin Phase 5 security changes inside this checkpoint.
