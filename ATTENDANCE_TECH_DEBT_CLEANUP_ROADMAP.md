...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, Phase 3B5, and Phase 3B6 are complete and merged. Phase 3B7A teacher-admin extraction is implemented on draft PR #33 and final normalized exact-head verification is pending. Phase 3B7B has not started.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B6 baseline has 43 Chromium tests. The Phase 3B7A branch adds one teacher-admin hostile-text/action regression, bringing the branch verification suite to 44 tests; do not call 44 the merged baseline until PR #33 is merged.
...
**Current checkpoint:** Phase 3B7A — teacher-admin extraction implemented on draft PR #33; final five-file normalization, exact-head verification, review, and explicit merge approval remain pending. Phase 3B7B has not started.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `fbe65f87b5a6a106ce1953a9d22a26a6793c7635`, the documentation-only Phase 3B6 closure merge from PR #32. Its parents are Phase 3B6 runtime merge `a915ebf897a29285594bd274725f00c3dfc1bea9` and approved docs head `7ade7ed515d1657f0f17f8240dd78cfdeb9aa8e7`.
- Phase 3B6 runtime PR #31 exact approved head `5c60d94f3b9dca395bce4f37f7c8066a031f3ce3` passed Phase 0A `34825996158`, Phase 0B `34825996159`, and Playwright `34825996150` with 43/43 tests, then merged as signed runtime commit `a915ebf897a29285594bd274725f00c3dfc1bea9`.
- Phase 3B7A approved scope is limited to `assets/js/teacher-admin.js`, `assets/js/main.js`, `tests/e2e/admin-management.spec.mjs`, `.github/workflows/phase0a-integrity.yml`, and this roadmap. `index.html`, signup/gate/recovery ownership, SQL/RPC/grant/RLS, hosted Auth, Science, Netlify, production data, and deployment remain unchanged.
- Current structural head `4c5eea1dea8f5e49b5635b95038021ba79d51f25` passed Phase 0A Integrity `34853746386`, Phase 0B Backend Contract `34853746557`, and Phase 1 Playwright `34853746392`; Playwright ran 44 Chromium tests with all 44 passing in 19.1s on Node `v22.23.2`, npm `10.9.8`, with 0 vulnerabilities. The new teacher-admin hostile-text/action regression, all existing teacher-admin/signup/recovery tests, all ten Attendance tests, student-management tests, and February/missing-register/dashboard/Term 1 + CSV safeguards passed.
- Earlier intermediate structural heads were intentionally rejected by Phase 0A for exact-source drift: one separator-newline mismatch and one lost final newline in `main.js`. Runtime assertions were never weakened; the source/proof was corrected and the exact structural gate now passes.
