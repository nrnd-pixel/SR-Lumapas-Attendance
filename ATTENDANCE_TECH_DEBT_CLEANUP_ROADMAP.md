...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, and Phase 3B5 are complete and merged. Phase 3B6 student management is implemented on a review branch and verification is pending; it is not merged.
...
- **3B6 — Student-management boundary:** move only current student roster, filtering/rendering, movement-history rendering, Transfer In dialog/defaults, Transfer Out dialog, Move Class dialog, and the three existing movement-submit workflows into `assets/js/student-management.js`. Reuse the existing Supabase client by passing it to `initStudentManagement()` together with the existing shared `schoolId()`, `adminClasses()`, and `showAdminMsg()` helpers; keep those helpers, shared state object, panel/tab orchestration, generic `[data-close]` wiring, and event-listener registration in `main.js`. Preserve exact RPC payloads/messages/DOM IDs and backend-owned movement/history semantics. Add browser regressions for escaped hostile pupil text plus search/class filtering/action-button integrity, and for Transfer In Brunei-date/class/reporting-group statistics defaults. Evolve Phase 0A so signed Phase 3B5 runtime merge `8a3b8173a1e576a8eee39df721bcfcc7a98ce971` is historical evidence and current Phase 3B6 is an exact deterministic extraction from signed docs-closure base `b4fbee48cc91fb9e60f4163c0fd59d1363f5f9db`. **Implemented on `cleanup/phase-3b6-student-management-v2`; verification pending, not merged.**
- **3B7+ — Remaining ownership split:** after 3B6 is separately verified and merged, impact-map teacher access, statistics/dashboard/reporting ownership, and finally orchestration/event wiring in small reversible checkpoints. Do not combine dependency migration, CSP hardening, backend security changes, UI redesign, stale-roster correctness work, or reporting consolidation with these structural moves.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B5 baseline has 41 Chromium tests. The Phase 3B6 branch adds two narrow student-management regressions and therefore targets 43 Chromium tests before merge.
...
- Stale async-response overwrites are guarded on cleanup `main` by request-serial/selection-key checks with deterministic browser coverage for the protected surfaces implemented in Phase 2C. The student-admin roster surface is a separately recorded pre-existing gap and is not silently included in those guarantees. This does not constitute production promotion; v0.7 remains the protected live baseline until an explicitly approved release.
...
**Current checkpoint:** Phase 3B6 — STUDENT MANAGEMENT EXTRACTION IMPLEMENTED; VERIFICATION PENDING. Not merged.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `b4fbee48cc91fb9e60f4163c0fd59d1363f5f9db`, the documentation-only Phase 3B5 closure merge from PR #29. Its parents are runtime `main` `8a3b8173a1e576a8eee39df721bcfcc7a98ce971` and docs head `58d1c7dfe19721883ec28ba8209c3bf112924ec1`.
- Phase 3B6 approved base: signed `main` `b4fbee48cc91fb9e60f4163c0fd59d1363f5f9db`.
- Working branch: `cleanup/phase-3b6-student-management-v2`. The originally named `cleanup/phase-3b6-student-management` already contained prior unsigned work and was deliberately left untouched rather than reused; the v2 branch starts from the exact signed approved base.
- Approved scope is exactly five files: add `assets/js/student-management.js`; modify only `assets/js/main.js`, `tests/e2e/admin-management.spec.mjs`, `.github/workflows/phase0a-integrity.yml`, and this roadmap. `index.html`, `assets/js/supabase-client.js`, `assets/js/app-state.js`, `assets/js/date-helpers.js`, `assets/js/ui-helpers.js`, `assets/js/auth-session.js`, `assets/js/attendance-register.js`, `netlify.toml`, repository SQL, and deployment configuration remain unchanged.
- Student-management module ownership is limited to roster load/render/filter, recent movement rendering, Transfer In dialog/defaults, Transfer Out/Move Class dialog setup, and the three existing movement-submit workflows. Existing `schoolId()`, `adminClasses()`, `showAdminMsg()`, shared state, panel/tab orchestration, generic dialog-close wiring, and event-listener registration remain in `main.js`.
- Existing movement RPCs and their server-side authorization/history semantics are unchanged: `attendance_admin_student_roster`, `attendance_admin_transfer_in`, `attendance_admin_transfer_out`, and `attendance_admin_move_class` remain authenticated-only SECURITY INVOKER functions that internally require school-admin authorization.
- Two browser regressions are added: hostile pupil text remains escaped while search/class filtering and generated action buttons stay correct; Transfer In keeps Brunei-today default, class options, Mainstream statistics inclusion, and SEN/UPK/PRAVOC exclusion. Full expected suite after this checkpoint is 43 Chromium tests.
- Fresh read-only live Supabase verification reconfirmed the latest Attendance migration remains `20260906125521 attendance_v12_term_ytd_reporting`; no live mutation was made. The newly documented direct-DML admin bypass risk and pre-existing student-roster stale-response race are explicitly deferred rather than mixed into this structural checkpoint.
- No Supabase SQL/RPC/grant/RLS, hosted Auth, Science, Netlify, production-data, or deployment mutation is part of Phase 3B6.
- The existing GitHub Actions warning that `actions/checkout@v4` / `actions/setup-node@v4` target deprecated Node 20 and are forced onto Node 24 remains separate workflow-maintenance debt; the application test runtime remains Node 22.23.2.
- No explicit production promotion has been performed; protected live v0.7 remains the release boundary.

**Production safeguard:** keep v0.7 live and unchanged during cleanup. Do not treat cleanup `main` or this branch as a production release.

**Next action:** open/review the focused Phase 3B6 PR, require Phase 0A, Phase 0B, the full 43-test Playwright suite, reporting invariants, exact diff review, and explicit merge approval. Do not merge automatically and do not begin Phase 3B7.

**Recommended thinking effort:** High.

## Change log

- **14 Sep 2026:** Phase 3B6 student-management impact map was completed and explicitly approved from exact signed `main` `b4fbee48cc91fb9e60f4163c0fd59d1363f5f9db`. Fresh read-only live Supabase verification confirmed the four student-management RPCs remain authenticated-only SECURITY INVOKER functions with school-admin checks and reconfirmed the latest Attendance migration `20260906125521`. It also surfaced broader authenticated direct table privileges on `students`/`enrolments` and `student_movements`, classified as Phase 5 security debt, plus a pre-existing frontend stale-roster response race classified for a separate correctness follow-up. The approved Phase 3B6 implementation extracts only student-management ownership into `assets/js/student-management.js`, keeps shared admin helpers/state/orchestration in `main.js`, and adds two narrow browser regressions. Because the originally named branch already contained prior unsigned work, a clean `cleanup/phase-3b6-student-management-v2` branch was created from the exact signed base and the old branch was left untouched. Verification and explicit merge approval remain pending; no SQL/RPC/grant/RLS/Auth/Science/Netlify/production-data/deployment mutation was made.
