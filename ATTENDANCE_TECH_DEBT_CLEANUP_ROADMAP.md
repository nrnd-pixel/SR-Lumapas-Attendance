...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, Phase 3B5, and Phase 3B6 are complete and merged. Phase 3B7 has not started.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B6 baseline has 43 Chromium tests, including the two Phase 3B6 student-management regressions.
...
**Current checkpoint:** Phase 3B6 — COMPLETE AND MERGED. Phase 3B7 has not started.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `a915ebf897a29285594bd274725f00c3dfc1bea9`, the Phase 3B6 runtime merge from PR #31. Its parents are previous signed `main` `b4fbee48cc91fb9e60f4163c0fd59d1363f5f9db` and approved PR head `5c60d94f3b9dca395bce4f37f7c8066a031f3ce3`.
- Phase 3B6 exact-head CI was fully green: Phase 0A Integrity `34825996158`; Phase 0B Backend Contract `34825996159`; Phase 1 Playwright `34825996150` with 43/43 Chromium tests in 17.5s, Node `v22.23.2`, npm `10.9.8`, and 0 vulnerabilities.
- All five student-management browser scenarios passed, including the two new Phase 3B6 regressions; all ten Attendance regressions and the 3A February, missing-register, admin-dashboard, and Term 1/CSV reporting safeguards also passed.
- Phase 3B6 merged exactly the approved five-file structural checkpoint: `assets/js/student-management.js`, `assets/js/main.js`, `tests/e2e/admin-management.spec.mjs`, `.github/workflows/phase0a-integrity.yml`, and this roadmap. No `index.html`, other extracted module, SQL/RPC/grant/RLS, hosted Auth, Science, Netlify, production-data, or deployment mutation was included.
- Student-management ownership now sits in `assets/js/student-management.js`; shared `schoolId()`, `adminClasses()`, `showAdminMsg()`, state, tab/panel orchestration, generic dialog-close wiring, and event-listener registration remain in `main.js`.
- The authenticated admin direct-DML bypass risk on student/enrolment/movement tables remains deferred to Phase 5. The pre-existing `attendance_admin_student_roster` stale-response race remains a separate correctness follow-up; neither was mixed into Phase 3B6.
- Superseded draft PR #30 was closed without merge; its earlier iterative branch remains only for auditability. PR #31 is the accepted Phase 3B6 runtime checkpoint.
