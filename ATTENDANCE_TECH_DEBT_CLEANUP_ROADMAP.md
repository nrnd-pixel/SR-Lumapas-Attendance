...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, Phase 3B5, Phase 3B6, Phase 3B7A, Phase 3B7B, and Phase 3B7C are complete and merged. Phase 3B7D+ has not started.
...
- **3B7D — Admin dashboard boundary:** after 3B7C is separately verified and merged, impact-map the school-wide admin dashboard as its own checkpoint because it depends on `schoolId()`, admin authorization, and cross-panel Dashboard → Statistics navigation. Do not implement it inside 3B7C.
- **3B7E — Term/YTD reports and CSV boundary:** after the earlier reporting surfaces are separately verified and merged, impact-map report options, Term/YTD loading/rendering, selection invalidation, and CSV/browser-download ownership as a separate checkpoint. Preserve the official 3A Term 1 fixture and missing-register semantics. Do not implement it inside 3B7C.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B7C baseline has 46 Chromium tests including the gender-incomplete monthly-statistics regression. Exact PR #37 normalized-head Phase 0A/0B and 46/46 Playwright verification passed before merge.
...
**Current checkpoint:** Phase 3B7C — COMPLETE AND MERGED. Phase 3B7D+ has not started.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `bf7379332a35f1ea9f859ea2ade6b6e67cdf3c73`, the Phase 3B7C runtime merge from PR #37. Its parents are prior signed cleanup `main` `1912512eb344b17fa0f219268becfdac31bba00d` and approved normalized PR head `0ee26371a67860c98c45d8c48b79b9a8ec3f2776`; tree `91337aa06661e772876ebf2a419935d00bce098e`; GitHub signature verification is valid.
- PR #37 approved normalized head `0ee26371a67860c98c45d8c48b79b9a8ec3f2776` was exactly one commit ahead / zero behind its signed base with exactly five changed files. Exact-head CI passed: Phase 0A Integrity `34913511240`, Phase 0B Backend Contract `34913511220`, and Phase 1 Playwright `34913511216`; Playwright ran 46 Chromium tests with all 46 passing in 18.1s on Node `v22.23.2`, npm `10.9.8`, with 0 vulnerabilities.
- The gender-incomplete browser regression proves the warning renders when gender is missing, male/female cumulative figures remain explicitly partial, total cumulative attendance remains complete at 398, possible attendance remains 425, the roster text remains correct, and the exact `attendance_monthly_class_stats` payload is unchanged. Existing 3A February, missing-register, dashboard, Term 1/YTD/CSV, stale-report-response, signup/recovery, all ten Attendance, student/teacher administration, and UI-helper coverage all remained green.
- Phase 3B7C extracts only monthly statistics frontend ownership into `assets/js/statistics.js`. `state.monthlyStats` and `requestSerial.monthlyStats` remain in `app-state.js`; statistics panel orchestration and event listeners remain in `main.js`; dashboard and Term/YTD/CSV ownership remain untouched.
- Fresh Phase 3B7C live read-only verification reconfirmed latest Attendance migration `20260906125521 attendance_v12_term_ytd_reporting`. `attendance_monthly_class_stats(p_class_id uuid,p_month date)` remains authenticated/service-role `STABLE SECURITY DEFINER`, performs an explicit authenticated-user check and `attendance.can_access_class(p_class_id)`, and keeps enrolment-date eligibility, school-day selection, counts-as-present semantics, missing-register handling, possible-attendance denominators, cumulative totals, gender completeness, and class authorization backend-owned. No Supabase mutation was made.
- Admin Dashboard remains deliberately deferred to Phase 3B7D because it depends on `schoolId()`, admin authorization, and Dashboard → Statistics navigation. Term/YTD rendering and CSV/browser-download ownership remain deliberately deferred to Phase 3B7E. The Check Approval, student-roster, and teacher-list stale-response gaps plus Phase 5 teacher direct-DML/role debt remain unchanged and out of scope.
- No explicit production promotion has been performed; protected live v0.7 remains unchanged.

**Production safeguard:** keep v0.7 live and unchanged during cleanup. Do not treat cleanup `main` as a production release.

**Next action:** perform a fresh Phase 3B7D Admin Dashboard impact map from exact signed `main` `bf7379332a35f1ea9f859ea2ade6b6e67cdf3c73`, re-verifying current repository and live Supabase ownership/security evidence before implementation. Do not implement until that map is reviewed and explicitly approved. Do not start Phase 3B7E, fix the recorded stale-response races, begin Phase 4 reporting consolidation, or begin Phase 5 security changes inside the structural impact map.

**Recommended thinking effort:** High.

## Change log

- **15 Sep 2026:** Phase 3B7C completed. PR #37 exact approved normalized head `0ee26371a67860c98c45d8c48b79b9a8ec3f2776` was exactly one commit ahead / zero behind signed base `1912512eb344b17fa0f219268becfdac31bba00d` with exactly five changed files. Exact-head gates were fully green: Phase 0A Integrity `34913511240`, Phase 0B Backend Contract `34913511220`, and Phase 1 Playwright `34913511216` with 46/46 Chromium tests in 18.1s, Node `v22.23.2`, npm `10.9.8`, and 0 vulnerabilities. The new gender-incomplete monthly-statistics regression passed alongside all signup/recovery, ten Attendance, student/teacher administration, missing-register, 3A February, admin-dashboard, Term 1/YTD, stale-report-response, CSV, and UI-helper safeguards. PR #37 then merged with the expected-head SHA guard as signed runtime commit `bf7379332a35f1ea9f859ea2ade6b6e67cdf3c73`, with parents `1912512eb344b17fa0f219268becfdac31bba00d` and `0ee26371a67860c98c45d8c48b79b9a8ec3f2776`, and tree `91337aa06661e772876ebf2a419935d00bce098e`. No SQL/RPC/grant/RLS/Auth/Science/Netlify/production-data/dashboard/Term-YTD/CSV/deployment mutation was made. Phase 3B7D+ has not started.
- **15 Sep 2026:** Phase 3B7C monthly-statistics impact mapping was completed and explicitly approved from exact signed `main` `1912512eb344b17fa0f219268becfdac31bba00d`, then branch `cleanup/phase-3b7c-statistics` and draft PR #37 were created. The approved boundary moves only `setStatsBanner()`, `loadMonthlyStats()`, and `renderMonthlyStats()` into new `assets/js/statistics.js`; `state.monthlyStats`, request serial ownership, statistics panel orchestration/listener wiring, Admin Dashboard, Term/YTD/CSV, Auth/recovery, backend contracts, Science, Netlify, production data, and deployment remain unchanged. Fresh read-only live Supabase verification reconfirmed latest Attendance migration `20260906125521 attendance_v12_term_ytd_reporting`, authenticated-only class-statistics access through `attendance.can_access_class`, and backend ownership of reporting denominators/missing-register/eligibility semantics. Four-file structural head `33db9b0429f0412731f79717473d61013660e508` passed Phase 0A `34912869909`, Phase 0B `34912869882`, and Playwright `34912869880` with 46/46 Chromium tests in 18.2s, Node `v22.23.2`, npm `10.9.8`, and 0 vulnerabilities. The new gender-incomplete regression passed alongside all protected reporting, recovery, Attendance, administration, and UI-helper tests. Final roadmap/five-file normalization and exact-current-head CI remain mandatory before merge; no Supabase/Auth/Science/Netlify/production-data/deployment mutation was made.
