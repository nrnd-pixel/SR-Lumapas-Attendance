...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, and Phase 3B3B are complete and merged. Phase 3B4 auth-session/password-recovery extraction is implemented on `cleanup/phase-3b4-auth-session`; verification and merge approval are pending.
...
- Phase 1 regression coverage is complete on cleanup `main` with 36 Chromium tests. The active Phase 3B4 branch adds two narrow auth-session regressions (recovery-session sign-out failure and external `SIGNED_OUT` routing), so the proposed exact-head suite is 38 tests if the checkpoint verifies successfully.
...
- Phase 3B4 is startup-order sensitive: recovery-link detection and `PASSWORD_RECOVERY` must continue to outrank normal `getSession()` routing. The checkpoint therefore keeps one Supabase client and proves the source transformation plus real-browser recovery behavior; it must not be combined with teacher signup/access redesign.
...
**Current checkpoint:** Phase 3B4 — AUTH SESSION/PASSWORD-RECOVERY EXTRACTION IMPLEMENTED ON BRANCH; exact-head verification and merge approval pending.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed Phase 3B4 implementation base on cleanup `main`: `cf753418b51e92ff91b223d448bbee2a783d3736` (PR #25 documentation closure merge).
- Active branch: `cleanup/phase-3b4-auth-session`, created from that exact base.
- Phase 3B1 branch `cleanup/phase-3b1-supabase-client` was created from exact base `a4eef41c5fa0dd6d807927b187b697ff03ae9afe`; approved head `7c5e4e9c3e61b9394bb48850333d1cdacedeb648` merged via PR #18 as signed runtime commit `ad2026b1056e311e41b50746601398938b73e16a`; docs closure PR #19 then moved `main` to `0794ac1e63c9d516f4f69c3cde7185b4822b98fa`. Phase 3B2 branch `cleanup/phase-3b2-shared-state` was created from that exact base; approved head `70abada98f17c9eb37209d315aa219005ce5f8ec` merged via PR #20 as signed runtime commit `ece37a0cfeaf996a20c10a3f0cc638d35cea3c56`. Docs closure PR #21 then moved Git `main` to `363c581ec42dda337902dad147a8fbe7b18a155e`. Phase 3B3A branch `cleanup/phase-3b3a-date-helpers` was created from that exact signed main; approved head `f52617c479e405fcbae0bef21b67dfbcc7485f4d` merged via PR #22 as signed runtime commit `8427442405d6d5a20debe430163b21c243f167ae`; docs closure PR #23 then moved Git `main` to `20f3088bb5a2f0161409ad04ce486c94c40a1801`. Phase 3B3B branch `cleanup/phase-3b3b-ui-helpers` was created from that exact signed main; approved head `60b8be26675370e086fef620a01a9c798f4e26b3` merged via PR #24 as signed runtime commit `53ed00e309f221a0f19e2ec446a41e01144177b0`; docs closure PR #25 then moved signed Git `main` to `cf753418b51e92ff91b223d448bbee2a783d3736`.
- Phase 3B4 adds `assets/js/auth-session.js` and moves only auth-session/password-recovery ownership out of `main.js`: recovery-link detection/state, shared entry-view hiding/login display, login submit, Forgot Password, recovery update/sign-out, app sign-out, auth-state events, and startup session routing. The module reuses the existing single Supabase client and calls the existing `enterApp` orchestration callback. Teacher signup, versioned user-bound pending-signup `localStorage`, teacher access gate/approval, RPC ownership, and bootstrap remain in `main.js`.
- Phase 0A now preserves the original frozen v1.0 checksum and historical Phase 3A/3B1/3B2/3B3A/3B3B transformations, verifies Phase 3B4 as an exact deterministic extraction from base `cf753418b51e92ff91b223d448bbee2a783d3736`, pins the exact `auth-session.js` module hash, asserts `index.html`, existing helper/client modules and `netlify.toml` are unchanged, and syntax-checks all six current JavaScript modules.
- Two proposed browser regressions extend auth coverage: a successful password update whose recovery-session sign-out fails must stay on Set New Password with no Attendance RPC, and an external `SIGNED_OUT` event must return an authorized session to login without additional Attendance RPCs. Full exact-head suite target: 38 Chromium tests.
- Fresh read-only live Supabase verification during the Phase 3B4 impact map confirmed 18 public Attendance RPCs, 11 SECURITY DEFINER RPCs, 18 Attendance/Private tables, 17 RLS-enabled tables, 50 Attendance RLS policies, and latest Attendance migration `20260906125521 attendance_v12_term_ytd_reporting`. No backend mutation was made; the known authenticated direct-DML risk remains deferred to Phase 5.
...
- User-provided live Netlify Build & deploy settings evidence on 13 September 2026 shows `Current repository: Not linked`. This has not been freshly re-opened during Phase 3B4 implementation; no Netlify configuration was changed and no production promotion was performed.
...
**Next action:** normalize the Phase 3B4 branch to a clean single-purpose history, open a draft PR, then verify the exact PR head with Phase 0A Integrity, Phase 0B Backend Contract, and the full Playwright suite. Review exact diff, comments/reviews, rollback and recovery invariants, then stop for explicit merge approval. Do not begin Phase 3B5 automatically.
