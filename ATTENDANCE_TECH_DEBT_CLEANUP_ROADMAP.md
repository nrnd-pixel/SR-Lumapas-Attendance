...
**Status:** In progress. Phase 3A, Phase 3B1, Phase 3B2, Phase 3B3A, Phase 3B3B, Phase 3B4, Phase 3B5, Phase 3B6, and Phase 3B7A are complete and merged. Phase 3B7B is implemented on draft PR #35 with structural verification green; final five-file normalization/current-head verification and explicit merge approval are pending. Phase 3B7C+ has not started.
...
- Phase 1 regression coverage is complete on cleanup `main`; the merged Phase 3B7A baseline has 44 Chromium tests, and the verified Phase 3B7B structural head has 45/45 Chromium tests including the rejected-request correction/resubmission regression. Final normalized-head verification remains mandatory before merge.
...
**Current checkpoint:** Phase 3B7B — IMPLEMENTED ON PR #35; structural verification is green. Final roadmap/five-file normalization, exact-head CI, final diff/review inspection, and explicit merge approval are pending.

- Canonical repository: `nrnd-pixel/SR-Lumapas-Attendance` (public).
- Exact signed cleanup `main`: `3259ad542b9b1816ab98aafa2a502b833ee69a65`, the Phase 3B7A documentation-closure merge from PR #34. Its parents are signed Phase 3B7A runtime merge `f9fb4f88a98f5e9f03a1d33b92ebe69ea19691ca` and approved documentation head `3bd00b8f02d287dcaaad1a7f48e3ec69e29a5a91`.
- Draft PR #35 structural head `6df42d8b5dc63ff8bf103aed6b0acf3f82c19dc4` contains the first four approved files (`assets/js/teacher-access.js`, `assets/js/main.js`, `tests/e2e/auth-access.spec.mjs`, `.github/workflows/phase0a-integrity.yml`) and passed Phase 0A Integrity `34905444884`, Phase 0B Backend Contract `34905444906`, and Phase 1 Playwright `34905444876`; Playwright ran 45 Chromium tests with all 45 passing in 14.8s on Node `v22.23.2`, npm `10.9.8`, with 0 vulnerabilities.
- The new rejected-request browser regression proves hostile-looking admin-note markup remains literal text, the rejected request pre-fills name/class/role correctly, the corrected class/role is sent in the exact `attendance_submit_teacher_request` payload, and the UI returns to Pending Approval. Existing signup isolation, recovery, all ten Attendance regressions, student/teacher admin, missing-register, 3A February, dashboard, Term 1/YTD, stale-report-response, CSV, and UI-helper coverage all remained green.
...
- The newly mapped Check Approval stale-response/UI-routing race remains a separate correctness follow-up. Existing teacher direct-DML/audit-bypass and generic active-role semantics remain Phase 5 debt. None is being mixed into Phase 3B7B.
...
**Next action:** add this roadmap as the fifth Phase 3B7B file, normalize PR #35 to one focused commit directly from signed `main` `3259ad542b9b1816ab98aafa2a502b833ee69a65`, rerun exact Phase 0A/0B and the full 45-test Playwright suite on that normalized head, inspect the final five-file diff/review surfaces, and seek explicit merge approval. Do not fix the Check Approval race or begin statistics/dashboard/reporting modularisation in this checkpoint.
